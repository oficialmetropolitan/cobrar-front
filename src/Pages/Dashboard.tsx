...
import { TrendingUp, AlertCircle, Wallet, Loader2 } from 'lucide-react';

interface EvolucaoMensal {
  mes: string;
  montante_mensal: number;
  capital_desembolsado: number;
  spread_realizado: number;
  inadimplencia_mensal: number;
}

export const DashboardEvolucaoCompleta: React.FC = () => {
  const [dadosMensais, setDadosMensais] = useState<EvolucaoMensal[]>([]);
  ...
  useEffect(() => {
    DashboardService.evolucaoMensal()
      ...
      .catch(e => console.error("Erro ao carregar evolução:", e))
  ...
  return (
    <div className="p-8 bg-[#F8FAFC] dark:bg-[#0B0F19] min-h-screen transition-colors duration-200">
      <h1 className="text-2xl font-black mb-8 text-slate-900 dark:text-white">
        Evolução de Carteira <span className="text-indigo-600 dark:text-indigo-400">Total</span>
      </h1>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900/30">
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                Mês Referência
              </th>
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                <div className="flex items-center gap-1"><Wallet size={12} /> Capital Desembolsado</div>
              </th>
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
            ...
            {dadosMensais.map((item, index) => (
              <tr key={index} className="hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors">
                <td className="px-8 py-5 font-bold text-slate-700 dark:text-slate-300">
                  {formatarData(item.mes)}
                </td>
                <td className="px-8 py-5 font-bold text-slate-900 dark:text-white">
                  {formatarMoeda(item.capital_desembolsado)}
                </td>
                <td className="px-8 py-5 font-bold text-slate-900 dark:text-white">
                  {formatarMoeda(item.montante_mensal)}
                </td>
                <td className="px-8 py-5 font-black text-indigo-600 dark:text-indigo-400">
                  {item.spread_realizado > 0 ? formatarMoeda(item.spread_realizado) : '—'}
                </td>
