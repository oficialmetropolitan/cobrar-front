...
const formtarData = (data: string) => {
  ...
  return (
    <div className="p-8 bg-[#F8FAFC] min-h-screen">
      <h1 className="text-2xl font-black mb-8">Evolução de Carteira <span className="text-indigo-600">Total</span></h1>

      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50">
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Mês Referência</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {dadosMensais.map((item, index) => (
              <tr key={index} className="hover:bg-slate-50 transition-colors">
                <td className="px-8 py-5 font-bold text-slate-700">{formtarData(item.mes)}</td>
                 <td className="px-8 py-5 font-bold text-slate-900">{formatarMoeda(item.valor_enviado_mensal)}</td>
                <td className="px-8 py-5 font-bold text-slate-900">{formatarMoeda(item.montante_mensal)}</td>
